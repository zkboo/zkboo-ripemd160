...
//!
//! RIPEMD-160 underlies Bitcoin's `HASH160 = RIPEMD160(SHA256(·))`, used for P2PKH, P2SH, and
//! P2WPKH addresses. It is an MD4-family design: little-endian message loading and length
//! padding, five 32-bit chaining words, and two parallel lines of 80 steps whose results are
//! cross-combined into the new chaining value.
//!
//! See "RIPEMD-160: A Strengthened Version of RIPEMD" (Dobbertin, Bosselaers, Preneel, 1996).
...
/// The round boolean function for step `j` (the left line uses `j`, the right line `79 - j`),
/// in single-AND form.
...
/// The MD4-style padding: message, 0x80, zeros, then the 64-bit bit length little-endian,
/// loaded into little-endian u32 words.
...
/// The digest is returned as an array of 20 [u8] words.
